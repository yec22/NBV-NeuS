...
class Warper:
    ...
    def bilinear_splatting(self, frame1: numpy.ndarray, mask1: Optional[numpy.ndarray], depth1: numpy.ndarray,
        ...
        if is_image:
            warped_frame2 = numpy.clip(warped_frame2, a_min=0.0, a_max=1.0)
...
def save_image(frame, save_name, rgb=False):
    frame = (frame.clip(min=0.0, max=1.0) * 255).astype(numpy.uint8)
    if rgb:
